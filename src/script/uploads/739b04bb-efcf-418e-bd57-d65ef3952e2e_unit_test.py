    ...
    class TestSQLQueries(unittest.TestCase):
        ...
        def test_select_second_query(self):
            try:
                ...
                logging.info(f"Query result: {result}")
                expected = [
                    ('French Riviera Famous Cities Scoot Coupe Self Drive Tour from Nice', 'France', None, 'Nice', Decimal('96.13')),
                    ('French Riviera in One Day', 'France', None, 'Nice', Decimal('89.26')),
                    ('Nice Small-Group Walking Food Tour with Local Specialties & Wine Tasting', 'France', None, 'Nice', Decimal('96.13')),
                    ('Bateaux Parisiens Seine River Cruise', 'France', None, 'Paris', Decimal('75.75')),
                    ('Disneyland Paris 1-Day Ticket', 'France', None, 'Paris', Decimal('76.90')),
                    ('Familia Sagrada Guided Tour', 'Spain', None, 'Barcelona', Decimal('70.03')),
                    ('Interactive Spanish Cooking Experience in Barcelona', 'Spain', None, 'Barcelona', Decimal('72.76')),
                    ('Sunset Jazz Cruise in Barcelona', 'Spain', None, 'Barcelona', Decimal('49.95'))
                ]
                self.assertEqual(result, expected)
            except Exception as e:
